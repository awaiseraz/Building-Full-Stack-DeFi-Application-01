...
//     it("should have correct name, symbol and totalSupply", async () => {
...
//     it("Should transfer token from one to another",
//         async () => {
//             expect(await token.balanceOf(deployer.address))
//                 .to.equal(toWei(1000000));
//             await token.connect(deployer)
//                 .transfer(addr1.address, toWei(5));
//             expect(await token.balanceOf(addr1.address))
//                 .to.equal(toWei(5));
//             expect(await token.balanceOf(deployer.address))
//                 .to.equal(toWei(999995));
//         });
...
describe("SimpleDeFiToken", () => {
  ...
  it("Should transfer token from one to another", async () => {
    ...
    await expect(token.connect(user1).transfer(user2.address, toWei(10)))
      .to.be.reverted; //With("ERC20: transfer amount exceeds balance");
    expect(await token.balanceOf(user1.address)).to.equal(toWei(5));
    expect(await token.balanceOf(user2.address)).to.equal(toWei(0));
  });
